...
class PureGoogleRouter extends PureRouter {
    ...
    private mountRoutes() {
        this.router.post("/fetch_from_sheets", verifyToken, this.handleFetchFromSheets);
        this.router.post("/write_to_sheets", verifyToken, this.handleWriteToSheets);
    }

    private handleFetchFromSheets = async (req: express.Request, res: express.Response) => {
        try {
    ...
    private handleWriteToSheets = async (req: express.Request, res: express.Response) => {
        try {
