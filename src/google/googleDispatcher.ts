...
export class GoogleDispatcher {
    ...
    public async populateDatabase() {
        ...
        for (const rawUser of peopleValues) {
            const mongoUser = await this.userDatabase.createNewUser(this.formatUser(rawUser));
        ...
        for (const rawEvent of eventValues) {
            await this.eventDatabase.createNewEvent(this.formatEvent(rawEvent, eventAttendees));
        }
    ...
    private formatForWriting(multipleObjects: any) {
        return multipleObjects.map(
            (singleObject: any) => {
                return Object.values(singleObject).map((value: any) => JSON.stringify(value));
            },
        );
    }
