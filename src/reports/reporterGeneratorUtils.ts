...
export async function getAllInactiveUsers(database: mongo.Db) {
    const allClaimedUsers = await database.collection(USERS_COLLECTION).find({ claimed: true }).toArray();
    const allLastEventsOfClaimedUsers = allClaimedUsers.map((user) => user.connections[user._id].slice(-1)).reduce(flatten);

    const allEventsFetched = await database.collection(EVENTS_COLLECTION).find({ _id: { $in: allLastEventsOfClaimedUsers.map(convertToMongoId) } }).toArray();
    const allInactiveUsers = allClaimedUsers.map((user) => {
        const finalUserEventID = user.connections[user._id].pop();
        if (finalUserEventID === undefined) {
            return undefined;
        }
        const finalEvent = allEventsFetched.find((event) => event._id.toString() === finalUserEventID.toString());
        const daysSinceLastEvent = differenceBetweenDates(new Date(finalEvent.date), new Date());
...
export function createEventsMailBody(eventsMadeInLast24Hours: any[], eventsInTwoDays: any[], allUsers: any[]) {
    return `
        <div>
...
export function createMailSubject() {
    return `AtlasP Text Report - ${new Date().toLocaleDateString()} (${process.env.NODE_ENV})`;
}
